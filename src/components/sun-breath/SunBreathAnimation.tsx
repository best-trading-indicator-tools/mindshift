...
const SunBreathAnimation: React.FC<Props> = ({ isInhaling, progress }) => {
  ...
  const currentRadius = mix(progress, CIRCLE_RADIUS, CIRCLE_RADIUS * 1.5);
      ...
      {isInhaling && (
        <Group>
          {rays.map((_, index) => {
            ...
            const rayLength = mix(progress, CIRCLE_RADIUS, CIRCLE_RADIUS * 3);
            const x1 = CENTER.x + Math.cos(angle) * currentRadius;
            const y1 = CENTER.y + Math.sin(angle) * currentRadius;
      ...
      {!isInhaling && (
        <Group>
          {clouds.map((_, index) => {
            ...
            const cloudDistance = mix(progress, CIRCLE_RADIUS * 3, CIRCLE_RADIUS);
